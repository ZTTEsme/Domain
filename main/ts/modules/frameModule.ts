...
export default class FrameModule implements Module {
  public constructor(private readonly authModule: AuthModule, private readonly homeModule: HomeModule) {}
  ...
  public async load(): Promise<void> {
    try {
      await window.initQnectFrame({
        ...
        updateAppConfigsHook: () => this.homeModule.getHomeInteractor().updateFavoriteApps(),
      });
    } catch (error) {
